...
export default defineComponent({
  ...
  setup() {
    ...
    const table = reactive({
      data: computed(() => {
        const data = storeCategoryJabatan.data.data.map((e, index) => {
          e.index = index + 1
          return e
        })
      ...
      isLoading: computed(() => storeCategoryJabatan.data.isLoading)
    })
    const form = storeCategoryJabatan.form
    const myForm = ref(null);
    onBeforeMount(() => storeCategoryJabatan.getData())
    return {
      columns,
      table,
      state,
      ...
      onFormShow(value, row) {
        if (value == 'add') {
          ...
          storeCategoryJabatan.dialog.form = true
          return
        }
        state.dialogTitle = 'Perubahan Data'
        storeCategoryJabatan.form = row
        storeCategoryJabatan.dialog.form = true
      }
    }
  }
...
<template>
  <div>
    <q-table title="Daftar Jabatan Surat Perintah Tugas" :rows="table.data" :loading="table.isLoading"
      :columns="columns" row-key="name">
      <template v-slot:top>
        <div>
          <p class="text-weight-bold text-uppercase q-ma-none">Kategori Jabatan</p>
        ...
        <q-btn label="Muat Ulang" class="q-mr-sm radius5" color="secondary" icon="refresh" flat
          @click="storeCategoryJabatan.getData()" />
      ...
      <q-card style="width: 700px; max-width: 80vw;" class="radius10">
        <q-item>
          ...
          <q-item-section side class="text-black">
            <q-item-label>KATEGORI JABATAN</q-item-label>
